...
/*
		...
		*/
	    ...
	    function insert($table, $data, $extra_sql = NULL) {
		  if ( !is_array( $data ) )
				return false;
				
			foreach ( array_values($data) as $field ) {
			  $param_fields[] = !is_numeric($field) ? "'" . mysql_escape_string($field) . "'" : addslashes($field);
			 }
			return $this->query("INSERT INTO `$table` (`" . implode( '`,`', array_keys($data) ) . "`) VALUES (" . implode( ",", $param_fields ) . ") ".$extra_sql);
		}
		...
		public	function update($table, $data, $where) {
			...
			foreach ( (array) $data as $field => $value ) {
				$form = !is_numeric($value) ? "'" . mysql_escape_string($value) . "'" : addslashes($value);
				$bits[] = "`$field` = {$form}";
			}
